...
void twoproton_filtered::Loop()
{
  ...
  TH1D* h_chi2p_p = new TH1D("h_chi2p_p","h_chi2p_p",15,0,300);
  TH1D* h_chi2mu_p = new TH1D("h_chi2mu_p","h_chi2mu_p",15,0,300);

  TH1D* h_chi2p_mu = new TH1D("h_chi2p_mu","h_chi2p_mu",15,0,300);
  TH1D* h_chi2mu_mu = new TH1D("h_chi2mu_mu","h_chi2mu_mu",15,0,300);

  TH1D* h_chi2p_pi = new TH1D("h_chi2p_pi","h_chi2p_pi",15,0,300);
  TH1D* h_chi2mu_pi = new TH1D("h_chi2mu_pi","h_chi2mu_pi",15,0,300);

  TH1D* h_chi2p_e = new TH1D("h_chi2p_e","h_chi2p_e",15,0,300);
  TH1D* h_chi2mu_e = new TH1D("h_chi2mu_e","h_chi2mu_e",15,0,300);

  TH1D* h_chi2p_other = new TH1D("h_chi2p_other","h_chi2p_other",15,0,300);
  TH1D* h_chi2mu_other = new TH1D("h_chi2mu_other","h_chi2mu_other",15,0,300);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) { //Loop over every entry in the file
	...
	neutrinos_else++;
      ...
      std::vector<float> good_trk_id; //vector used to define good daughters pdgs  
	...
	if(reco_3d_diff_start < 5.0 || reco_3d_diff_end < 5.0) {
	  ...
	  good_trk_id.push_back(id_pfp->at(i));
	}
	...
	h_overlay[3]->Fill(n_shower->at(i), wgt);
      ...
      //std::cout<<"Length: "<<mc_g4_pdg->size()<<std::endl;
      //for(int s=0; s < mc_g4_pdg->size(); s++){
      //	std::cout<<"PDG: "<<mc_g4_pdg->at(s)<<std::endl; 
       //}


      //for(int y = 0; y <  good_trk_id.push_back(id_pfp);

      //std::cout<<"something "<<id_pfp->size()<<std::endl; 
	...
	//std::cout<<"PDG with 0,1,2 g4: "<<mc_g4_pdg->at(v)<<std::endl;
	std::cout<<"PDG with trk ID: "<<mc_pdg->at(value)<<std::endl;
	std::cout<<"MC momentum: "<<mc_mom->at(value)<<std::endl;
	std::cout<<"reco momentum: "<<reco_mom->at(value)<<std::endl;

	if(mc_pdg->at(value) == -9999.) continue;

	if(mc_pdg->at(value) == 2212 || mc_pdg->at(value) == -2212){
	  h_chi2p_p->Fill(chi2_p_2->at(value),wgt);
	  h_chi2mu_p->Fill(chi2_mu_2->at(value),wgt);

	} else if(mc_pdg->at(value) == 211 || mc_pdg->at(value) == -211){
	  h_chi2p_pi->Fill(chi2_p_2->at(value),wgt);
	  h_chi2mu_pi->Fill(chi2_mu_2->at(value),wgt);
       
	} else if(mc_pdg->at(value) == 13 || mc_pdg->at(value) == -13){
	  h_chi2p_mu->Fill(chi2_p_2->at(value),wgt);
	  h_chi2mu_mu->Fill(chi2_mu_2->at(value),wgt);
          
	} else if(mc_pdg->at(value) == 11 || mc_pdg->at(value) == -11){
	  h_chi2p_e->Fill(chi2_p_2->at(value),wgt);
	  h_chi2mu_e->Fill(chi2_mu_2->at(value),wgt);
        
	} else {
	  h_chi2p_other->Fill(chi2_p_2->at(value),wgt);
          h_chi2mu_other->Fill(chi2_mu_2->at(value),wgt);
	    }
      }
