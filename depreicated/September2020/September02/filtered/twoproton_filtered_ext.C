  ...
  bool short_trk;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	if(j != shortest_trk_index && j != longest_trk_index){ //second shortest track/second longest track
	  if (_debug) std::cout<<"Start of the Second Shortest Track: "<<good_trk_start_x.at(j)<<" , "<<good_trk_start_y.at(j)<<" , "<<good_trk_start_z.at(j)<<std::endl;
	  if (_debug) std::cout<<"End of the Second Shortest Track: "<<good_trk_end_x.at(j)<<" , "<<good_trk_end_y.at(j)<<" , "<<good_trk_end_z.at(j)<<std::endl;
	  ...
	  if (_debug) std::cout<<"Start of the Shortest Track: "<<good_trk_start_x.at(j)<<" , "<<good_trk_start_y.at(j)<<" , "<<good_trk_start_z.at(j)<<std::endl;
	  if (_debug) std::cout<<"End of the Shortest Track: "<<good_trk_end_x.at(j)<<" , "<<good_trk_end_y.at(j)<<" , "<<good_trk_end_z.at(j)<<std::endl;
	  if((good_trk_start_x.at(j) <= xmin || good_trk_start_x.at(j) >= xmax || good_trk_end_x.at(j) <= xmin || good_trk_end_x.at(j) >= xmax)
             || (good_trk_start_y.at(j) <= ymin || good_trk_start_y.at(j) >= ymax || good_trk_end_y.at(j) <= ymin || good_trk_end_y.at(j) >= ymax)
             || (good_trk_start_z.at(j) <= zmin || good_trk_start_z.at(j) >= zmax || good_trk_end_z.at(j) <= zmin || good_trk_end_z.at(j) >= zmax)) {
	    short_trk = false;
      ...
      if (_debug) std::cout<<"Value of Second_trk for this event: "<<second_trk<<std::endl;
      if (_debug) std::cout<<"Value of short_trk for this event: "<<short_trk<<std::endl;
      ...
      std::cout<<"Size of total dedx: "<<total_dedx_1->size()<<std::endl;
