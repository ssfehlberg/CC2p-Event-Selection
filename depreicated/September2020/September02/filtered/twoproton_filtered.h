...
// from TTree tree/
// found on file: /uboone/data/users/sfehlber/2020/Sep2020/Sep02/overlay_filtered.root
//////////////////////////////////////////////////////////
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/uboone/data/users/sfehlber/2020/Sep2020/Sep02/overlay_filtered.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/uboone/data/users/sfehlber/2020/Sep2020/Sep02/overlay_filtered.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("/uboone/data/users/sfehlber/2020/Sep2020/Sep02/overlay_filtered.root:/TwoProtonAna");
...
void twoproton_filtered::Write_Histograms(){
  ...
  for(int i=0; i< number; i++){
    for(int j=0; j < number2; j++){ //my stuff	
       ...
       h_vtx_y[i][j]->Write();
       h_vtx_z[i][j]->Write();
       h_vtx_x_mc[i][j]->Write();
       h_vtx_y_mc[i][j]->Write();
       h_vtx_z_mc[i][j]->Write();
       h_vtx_x_mc_sce[i][j]->Write();
       h_vtx_y_mc_sce[i][j]->Write();
       h_vtx_z_mc_sce[i][j]->Write();
       h_q2[i][j]->Write();
       h_X[i][j]->Write();
       h_Y[i][j]->Write();
       h_Pt[i][j]->Write();
    ...
    for(int j=0; j< number3; j++){//raquel's stuff
      h_vtx_x_raquel[i][j]->Write();
      h_vtx_y_raquel[i][j]->Write();
      h_vtx_z_raquel[i][j]->Write();
      h_vtx_x_mc_raquel[i][j]->Write();
      h_vtx_y_mc_raquel[i][j]->Write();
      h_vtx_z_mc_raquel[i][j]->Write();
      h_vtx_x_mc_sce_raquel[i][j]->Write();
      h_vtx_y_mc_sce_raquel[i][j]->Write();
      h_vtx_z_mc_sce_raquel[i][j]->Write();
      h_q2_raquel[i][j]->Write();
      h_X_raquel[i][j]->Write();
      h_Y_raquel[i][j]->Write();
      h_Pt_raquel[i][j]->Write();
    }
  }
