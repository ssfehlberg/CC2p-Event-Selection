...
void twoproton_eff::Loop()
{
  ...
  h_num_overlay[2] = new TH1D(Form("h_num_overlay%s",total[2]),Form("h_num_overlay%s",total[2]),20,0,1);
  h_denom_overlay[2] = new TH1D(Form("h_denom_overlay%s",total[2]),Form("h_denom_overlay%s",total[2]),20,0,1); 
  ...
  float_t zmax = 1036.8 - FV_edge;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) { //Loop over every entry in the file
	...
	truthcntr++;
	 ...
	 if((mc_n_threshold_muon + mc_n_threshold_pionpm +  mc_n_threshold_proton) == 3){  
	 ...
	 if(good_trk.size() == 3 && (mc_ccnc == 0 && mc_nupdg == 14 && mc_n_threshold_proton == 2 && mc_n_threshold_pion0 == 0 && mc_n_threshold_pionpm == 0 && fv == true)){
	   num_cc2p++;
	 }
	 ...
	 if(good_trk.size() == 3 && ((mc_n_threshold_muon + mc_n_threshold_pionpm +  mc_n_threshold_proton) == 3)){
   ...
   std::cout << "[EFFICIENCY] Rough Efficiency Estimate For CC2p: "<<num_cc2p<<std::endl;//float(100.*(float(num_cc2p)/float(denom_cc2p)))<<"%"<<std::endl;
