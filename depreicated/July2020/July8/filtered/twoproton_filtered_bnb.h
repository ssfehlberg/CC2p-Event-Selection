...
// from TTree tree/
// found on file: /uboone/data/users/sfehlber/2020/April2020/Apr21/filtered/bnb.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
 ...
 private:
   ...
   static const int  number=2; //before and after selection                                                                                             
   const char * point[number] ={"_before_selection","_after_selection"}; //this defines histograms before and after the selection          
 
   TH1D* h_vtx_x[number]; //reco x                                                                                                           
   TH1D* h_vtx_y[number]; //reco y                                                                                                            
   TH1D* h_vtx_z[number]; //reco z  
   TH1D* h_chi2_p0[number]; //chi2 of the proton on plane 0
   TH1D* h_chi2_p1[number];//chi2 of the proton on plane 1 
   TH1D* h_chi2_p2[number];//chi2 of the proton on plane 2 
   TH1D* h_chi2_mu0[number];//chi2 of the muon on plane 0  
   TH1D* h_chi2_mu1[number];//chi2 of the muon on plane 1  
   TH1D* h_chi2_mu2[number];//chi2 of the muon on plane 2  
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/uboone/data/users/sfehlber/2020/April2020/Apr21/filtered/bnb.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/uboone/data/users/sfehlber/2020/April2020/Apr21/filtered/bnb.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("/uboone/data/users/sfehlber/2020/April2020/Apr21/filtered/bnb.root:/TwoProtonAna");
...
void twoproton_filtered_bnb::Define_Histograms(){
  ...
  for(int i=0; i< number; i++){
    ...
    h_chi2_p0[i]=new TH1D(Form("h_chi2_p0%s_bnb",point[1]),Form("h_chi2_p0%s_bnb",point[i]),100,-1000,1000);
    h_chi2_p1[i]=new TH1D(Form("h_chi2_p1%s_bnb",point[1]),Form("h_chi2_p1%s_bnb",point[i]),100,-1000,1000);
    h_chi2_p2[i]=new TH1D(Form("h_chi2_p2%s_bnb",point[1]),Form("h_chi2_p2%s_bnb",point[i]),100,-1000,1000);
    h_chi2_mu0[i]=new TH1D(Form("h_chi2_mu0%s_bnb",point[1]),Form("h_chi2_mu0%s_bnb",point[i]),100,-1000,1000);
    h_chi2_mu1[i]=new TH1D(Form("h_chi2_mu1%s_bnb",point[1]),Form("h_chi2_mu1%s_bnb",point[i]),100,-1000,1000);
    h_chi2_mu2[i]=new TH1D(Form("h_chi2_mu2%s_bnb",point[1]),Form("h_chi2_mu2%s_bnb",point[i]),100,-1000,1000);
...
void twoproton_filtered_bnb::Fill_Histograms(int i){
  ...
  h_chi2_p0[i]->Fill(chi2_p_0,wgt);
  h_chi2_p1[i]->Fill(chi2_p_1,wgt);
  h_chi2_p2[i]->Fill(chi2_p_2,wgt);
  h_chi2_mu0[i]->Fill(chi2_mu_0,wgt);
  h_chi2_mu1[i]->Fill(chi2_mu_1,wgt);
  h_chi2_mu2[i]->Fill(chi2_mu_2,wgt);
