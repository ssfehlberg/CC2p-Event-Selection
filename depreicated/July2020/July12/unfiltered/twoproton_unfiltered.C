  ...
  bool short_trk;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) { //Loop over every entry in the file
	...
	neutrinos_else++;
      ...
      std::vector<float> good_pdg; //vector with the pdg codes of the daughters
      std::vector<float> good_theta; //vector with the reconstructed theta
	...
	good_pdg.push_back(float(mc_pdg->at(i)));
	good_theta.push_back(float(mc_theta->at(i)));
	...
	if(j != shortest_trk_index && j != longest_trk_index){ //second shortest track/second longest track
      ...
      std::cout<<"Size of pdg: "<<good_pdg.size()<<std::endl;
      std::cout<<"Size of theta: "<<good_theta.size()<<std::endl;
      
      /*
      for(int i = 0; i < good_pdg.size(); i++){
	std::cout<<"Value of i: "<< i << std::endl;
	std::cout<<"Value of good_pdg at i: "<<good_pdg.at(i)<<std::endl;
	std::cout<<"Value of good_theta at i: "<<good_theta.at(i)<<std::endl;
	std::cout<<"Value of good_trk at i: "<<good_trk.at(i)<<std::endl;
      }
      */
