...
twoproton_pelee::twoproton_pelee(TTree *tree) : fChain(0) 
{
  ...
  if(response == 0){
    ...
    const char* files[number_of_files] = {"/uboone/data/users/apapadop/searchingfornues/high_stat_prodgenie_bnb_nu_overlay_DetVar_Run3_CV_reco2_reco2.root",
					  ...
					  "/uboone/data/users/apapadop/searchingfornues/high_stat_prodgenie_bnb_nu_overlay_DetVar_Run3_Overlay_reco2_reco2.root"};

    const char* samples[number_of_files] = {"detVar_CV","detVar_LY_Attenuation","detVar_LY_Down","detVar_LY_Rayleigh","detVar_Recombination","detVar_SCE","detVar_ThetaXZ","detVar_ThetaYZ","detVar_X","detVar_YZ","detVar_Overlay"};
    std::vector<double> pot_weights = {0.206,0.196,0.192,0.207,0.413,0.411,0.204,0.193,0.204,0.192,0.439}; //vector of pot weights for each file 
...
double twoproton_pelee::apply_cv_correction_weights( double pot_wgt, double map_weight){
  ...
  if(response == 0){
    ...
    //Flux, ReInt, xsr_scc_Fa3_SCC, xsr_scc_Fv3_SCC, and rootinofix
  } else if (response == 1 || response == 2 || response == 13 || response == 14 || response == 15){
    ...
    //GENIE All,fFCCQE Shape, Decay Angle, Norm CCcoh, Norm NCCoh, ThetaDelta2NRad, Theta Delta2npi, VecFFCCQEshape, XSecShape_CCMEC, RPA_CCQE
  } else {
  ...
  std::cout<<"[VALUE OF THE EVENT WEIGHT]"<<event_wgt<<std::endl;
  return event_wgt;
...
//THIS IS SPECIFIC TO MY BREAKDWON
//i=> Which universes                                                                                                                                                                                                                                                  
