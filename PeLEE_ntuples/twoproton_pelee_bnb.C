...
void twoproton_pelee_bnb::Loop()
{
  ...
  if(use_xsec_binning == true){
    tfile = new TFile(Form("root_files/%s/histograms_pelee_xsec_bnb.root",directory),"RECREATE"); //root file
  }else{
    tfile = new TFile(Form("root_files/%s/histograms_pelee_bnb.root",directory),"RECREATE"); //root file   
  }
